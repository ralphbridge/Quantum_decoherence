...
E=1.6e3; % 380 electron energy in eV
E_loss=0; % (68 for 22 um and 545 for 100 um) Energy loss in eV

E_l=E-E_loss;

%D=1e-12; % Laser beam waist
d=100e-9; % Periodicity of the grating
kd=2*pi/d;

w1=1e-6; % (6.7-22 microns) FWHM of incoherent gaussian slit width
sigma1=w1/(2*sqrt(2*log(2)));
w2=2e-6; % FWHM of collimating gaussian slit width
sigma2=w2/(2*sqrt(2*log(2)));
w3=5e-6; % FWHM of detection gaussian slit width
sigma3=w3/(2*sqrt(2*log(2)));
%l0=24.13-2; % distance from first collimating slit to second collimating slit (9.5")
l0=24.13e-2;
%l1=2.5e-2; % 6.35 cm distance from second collimating slit to grating (2.5")
l1=6.35e-2;
%l2=24.13e-2; % distance from grating to screen (?)
l2=24e-2;

N0=11; % 101 Number of points for the incoherent source slit x-coordinate
if N0==1
    x0=0;
    dx0=w1/N0;
else
...
xmax1=2e-6;
N1=5001; % 10001 Number of points for the grating (laser) x-coordinate
x1=linspace(-xmax1,xmax1,N1);
dx1=xmax1/(N1-1);
x1=x1';

xmax2=200e-6; % 100 microns to see only first and second diffraction orders
N2=3001; % 6001 Number of points for the detection screen x-coordinate
x2=linspace(-xmax2,xmax2,N2);
dx2=xmax2/(N2-1);
x2=x2';

Grating=0.5*(1+square(kd*(x1+d/4))); % ponderomotive potential
...
for iter=1:length(x0)
    ...
    Psi_sslit=zeros(size(x1)); % initializing wavefunction before laser interaction
...
%     subplot(2,2,2)
%     plot(x01*1e9,conj(Psi_slit).*Psi_slit)
%     grid on
%     xlabel('First collimating slit position $x_{01}\ n m$','fontsize',15,'interpreter','latex','interpreter','latex')
        
    % This section computes the wf propagation from the second (collimating) slit to the laser
    
    v=sqrt(2*(E_l*q)/m); % electron velocity with energy loss
    %t=D/v; % total interaction time with energy loss

    Psi_sslit=propagate(Psi_slit,x01,dx01,x1,l1,v);

%     subplot(2,2,3)
%     plot(x1*1e6,conj(Psi_sslit).*Psi_sslit)
%     grid on
%     xlabel('Second collimating slit position $x_{1}\ \mu m$','fontsize',15,'interpreter','latex','interpreter','latex')
    
    for i=1:length(x1) % Right before the laser beam
        for j=1:length(x1)
            rhok(i,j)=conj(Psi_sslit(i))*Psi_sslit(j);
    ...
    Psi_grate=Psi_sslit.*Grating;
%     subplot(2,2,4)
%     plot(x1*1e6,conj(Psi_grate).*Psi_grate/(max(conj(Psi_grate).*Psi_grate)))
%     grid on
% %     hold on
% %     plot(x1*1e6,Grating)
%     xlabel('Second collimating slit position $x_{1}\ \mu m$','fontsize',15,'interpreter','latex','interpreter','latex')
